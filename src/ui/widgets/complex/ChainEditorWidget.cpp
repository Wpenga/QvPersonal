...
ChainEditorWidget::ChainEditorWidget(std::shared_ptr<NodeDispatcher> dispatcher, QWidget *parent) : QWidget(parent), dispatcher(dispatcher)
{
    ...
    connect(dispatcher.get(), &NodeDispatcher::OnFullConfigLoadCompleted, [this]() {
        if (!chains.isEmpty())
            currentChainOutbound = chains.first();
        ShowChainLinkedList();
    });
    //
...
void ChainEditorWidget::OnDispatcherChainedOutboundCreated(std::shared_ptr<OutboundObject> data, QtNodes::Node &node)
{
    outboundNodes[data->name] = node.id();
...
void ChainEditorWidget::ShowChainLinkedList()
{
    ...
    if (!currentChainOutbound)
        return;
    const auto &outbounds = currentChainOutbound->chainSettings.chains;
    for (auto index = 0; index < outbounds.count() - 1; index++)
    {
        ...
        for (const auto &tag : { inTag, outTag })
        {
            ...
            {
                QvBaselib->Warn(tr("Chain Editor"), tr("Could not find outbound tag: %1, The chain may be corrupted").arg(tag));
...
{
    currentChainOutbound = chains[chainComboBox->itemText(arg1)];
    ShowChainLinkedList();
}
...
void ChainEditorWidget::TrySaveChainOutboudData(const QUuid &ignoredConnectionId)
{
    if (!currentChainOutbound)
    {
        QvBaselib->Warn(tr("Chain Editor"), tr("Please Select a Chain"));
        return;
    }
    ...
    else
    {
        ...
        currentChainOutbound->chainSettings.chains = list;
    }
}
...
void ChainEditorWidget::OnDispatcherChainedOutboundDeleted(const OutboundObject &data)
{
...
void ChainEditorWidget::OnDispatcherChainCreated(std::shared_ptr<OutboundObject> data)
{
    const auto displayName = data->name;
...
void ChainEditorWidget::OnDispatcherChainDeleted(const OutboundObject &data)
{
    const auto displayName = data.name;
...
void ChainEditorWidget::OnDispatcherObjectTagChanged(NodeItemType mode, const QString originalTag, const QString newTag)
{
    if (mode == NodeItemType::OUTBOUND)
    {
        ...
        {
            if (chain->chainSettings.chains.contains(originalTag))
                chain->chainSettings.chains.replace(chain->chainSettings.chains.indexOf(originalTag), newTag);
        }
    }
}
